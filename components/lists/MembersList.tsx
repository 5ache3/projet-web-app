...
export default function MembersList({isOpen,onClose,users,u_id,u_role,p_id}:{isOpen:boolean,onClose:()=>void,users:User[],u_id:string,p_id:string,u_role?:string}) {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
        ...
        <DialogContent className=' m-auto flex flex-col p-3 pt-15 px-10 bg-mainbg-2 w-full text-white overflow-auto h-150'>
            {users.map((user,index) => (
                    <UserCard key={index} user={user} u_id={u_id} u_role={u_role} p_id={p_id}/>
                  ))}
        </DialogContent>
    </Dialog>
  )
